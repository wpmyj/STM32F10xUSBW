...
*例程			:
*******************************************************************************/
void USART_DMA_ConfigurationBAC(USART_TypeDef* USARTx,u32 USART_BaudRate,u8 NVICPreemptionPriority,u8 NVIC_SubPriority,u32 *RXDBuffer,u32 *TXDBuffer,u32 BufferSize)	//USART_DMA配置
{
	//1)**********定义变量	
	DMA_InitTypeDef DMA_Initstructure;
...
//    va_end( ap );
	...
	else if(USARTx==UART4)
	{
		...
		DMA2_Channel5->CMAR =(u32)USART_BUFFER;							//发送缓冲区	
...
//	ErrorStatus status = ERROR;
	...
	if(USARTx==USART1)
	{
		if(USART_GetFlagStatus(USART1,USART_IT_IDLE))
		{
			...
			DMA1_Channel5->CNDTR=USART_BUFFERSIZE;														//重新设置接收数据个数			
	...
	{
		if(USART_GetFlagStatus(USART2,USART_IT_IDLE))
		{
	...
	{
		if(USART_GetFlagStatus(USART3,USART_IT_IDLE))
		{
	...
	{
		if(USART_GetFlagStatus(UART4,USART_IT_IDLE))
		{
	...
	{
			USART_ClearITPendingBit(UART5,USART_IT_IDLE); 										//清除空闲串口标志位
			DMA_Cmd(DMA1_Channel5,DISABLE);    																//关闭接收DMA
			num = USART1->DR; 																								//读出数据以完成清除标志			
			num = USART_BUFFERSIZE -  DMA_GetCurrDataCounter(DMA1_Channel5);	//得到真正接收数据个数
			DMA1_Channel5->CNDTR=USART_BUFFERSIZE;														//重新设置接收数据个数			
...
//	ErrorStatus status = ERROR;
	...
	if(USARTx==USART1)
	{
		if(USART_GetITStatus(USART1,USART_IT_IDLE)||USART_GetFlagStatus(USART1,USART_IT_IDLE))
		{
			USART_ClearITPendingBit(USART1,USART_IT_IDLE); 										//清除空闲串口标志位
			USART_ClearFlag(USART1,USART_IT_IDLE); 										//清除空闲串口标志位
			DMA_Cmd(DMA1_Channel5,DISABLE);    																//关闭接收DMA
			num = USART1->DR; 																								//读出数据以完成清除标志			
			num = USART_BUFFERSIZE -  DMA_GetCurrDataCounter(DMA1_Channel5);	//得到真正接收数据个数
			DMA1_Channel5->CNDTR=USART_BUFFERSIZE;														//重新设置接收数据个数			
	...
	{
		if(USART_GetITStatus(USART2,USART_IT_IDLE)||USART_GetFlagStatus(USART2,USART_IT_IDLE))
		{
			USART_ClearITPendingBit(USART2,USART_IT_IDLE); 										//清除空闲串口标志位
			USART_ClearFlag(USART2,USART_IT_IDLE); 										//清除空闲串口标志位
			DMA_Cmd(DMA1_Channel6,DISABLE);    																//关闭接收DMA
			num = USART1->DR; 																								//读出数据以完成清除标志			
			num = USART_BUFFERSIZE -  DMA_GetCurrDataCounter(DMA1_Channel5);	//得到真正接收数据个数
			DMA1_Channel5->CNDTR=USART_BUFFERSIZE;														//重新设置接收数据个数			
	...
	{
		if(USART_GetITStatus(USART3,USART_IT_IDLE)||USART_GetFlagStatus(USART3,USART_IT_IDLE))
		{
			USART_ClearITPendingBit(USART3,USART_IT_IDLE); 										//清除空闲串口标志位
			USART_ClearFlag(USART3,USART_IT_IDLE); 										//清除空闲串口标志位
			DMA_Cmd(DMA1_Channel3,DISABLE);    																//关闭接收DMA
			num = USART1->DR; 																								//读出数据以完成清除标志			
			num = USART_BUFFERSIZE -  DMA_GetCurrDataCounter(DMA1_Channel5);	//得到真正接收数据个数
			DMA1_Channel5->CNDTR=USART_BUFFERSIZE;														//重新设置接收数据个数			
	...
	{
		if(USART_GetITStatus(UART4,USART_IT_IDLE)||USART_GetFlagStatus(UART4,USART_IT_IDLE))
		{
			USART_ClearITPendingBit(UART4,USART_IT_IDLE); 										//清除空闲串口标志位
			USART_ClearFlag(UART4,USART_IT_IDLE); 										//清除空闲串口标志位
			DMA_Cmd(DMA2_Channel3,DISABLE);    																//关闭接收DMA
			num = UART4->DR; 																								//读出数据以完成清除标志			
			num = USART_BUFFERSIZE -  DMA_GetCurrDataCounter(DMA2_Channel3);	//得到真正接收数据个数
			DMA2_Channel3->CNDTR=USART_BUFFERSIZE;														//重新设置接收数据个数			
			DMA_Cmd(DMA2_Channel3,ENABLE);  																	//开启接收DMA
			return num;
		}
	}
...
*例程			:
*******************************************************************************/
u32 USART_RX_FlagClearbac(USART_TypeDef* USARTx)				//清除串口接收中断标志
{
//	ErrorStatus status = ERROR;
	u32 num=0;
